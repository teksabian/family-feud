...
Requirements:
- Questions must start with "Name something...", "Name a...", "Name a place...", "Name a reason...", "Tell me something...", or similar Survey Says phrasing
...
# ===== COUNTRY SAYS CONFIGURATION =====

COUNTRYSAYS_NUM_ANSWERS = 7

COUNTRYSAYS_ROUNDS_CONFIG = [
    {"round": i, "answers": 7} for i in range(1, 13)
]

COUNTRYSAYS_GENERATION_PROMPT = """You are a game show writer for "The Country Says" — a fill-in-the-blank survey game.

Generate {num_rounds} fill-in-the-blank prompts. Each prompt asks what people would say about a topic. For each prompt, provide exactly 7 answers ranked from most popular to least popular.

Requirements:
- Prompts should start with "The country says..." or similar phrasing
- Answers should be single words or very short phrases (1-3 words max)
- Answers should feel like real survey results — common answers many people would give
- Mix of topics: everyday life, food, travel, relationships, work, pop culture, holidays
- Fun, debatable topics that work for adults at a bar or community event
- No duplicate answers within a prompt
- Answers ranked by popularity (most common first)
...
{{
  "rounds": [
    {{
      "prompt": "The country says... the worst thing to forget on a road trip is ____",
      "answers": ["Phone", "Wallet", "Charger", "Sunglasses", "Snacks", "Keys", "Toothbrush"]
    }}
  ]
}}

Generate exactly {num_rounds} rounds."""
